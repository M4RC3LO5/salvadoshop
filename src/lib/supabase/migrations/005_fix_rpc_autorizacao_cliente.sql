-- ============================================================
-- SalvadoShop — Correção de autorização na RPC de pedido
-- Migração: 005_fix_rpc_autorizacao_cliente.sql
-- Criado em: 2026-07-03
-- Contexto: Bloco 6 — correção de autorização na RPC de pedido.
-- A função criar_pedido_com_estoque é SECURITY DEFINER e tinha
-- GRANT EXECUTE para o role authenticated, mas confiava cegamente
-- no parâmetro p_cliente_id sem checar contra auth.uid(). Como
-- SECURITY DEFINER contorna a RLS de INSERT em `pedidos` (que exige
-- cliente_id = auth.uid()), qualquer usuário autenticado — inclusive
-- sessão anônima — podia chamar essa RPC diretamente via
-- supabase.rpc() no client e criar pedidos / debitar estoque em
-- nome de outro cliente_id.
-- ============================================================
